...
class EMAC(nn.Module):
    ...
    def generate_random_masks(
        ...
        # task_sampling_dist = torch.zeros(B, 2).uniform_(0, 1).to(device)
        # task_sampling_dist[:, 0] = task_sampling_dist[:, 0].uniform_(0.7, 1)
        # task_sampling_dist[:, 1] = 1 - task_sampling_dist[:, 0]
