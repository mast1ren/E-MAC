...
class PWCDCNet(nn.Module):
    ...
    def warp(self, x, flo):
        ...
        # print(vgrid.type(), x.type())
        output = nn.functional.grid_sample(x, vgrid)
        # mask = torch.autograd.Variable(torch.ones(x.size())).cuda()
        mask = torch.ones(x.size(), device=device)
        # mask = torch.ones(x.size(), device=device).half()
        mask = nn.functional.grid_sample(mask, vgrid)
    ...
    def forward(self, x):
        ...
        # print(c25.shape, up_flow6.shape)
        warp5 = self.warp(c25, up_flow6 * 0.625).to(c15.dtype)
        # print(c15.dtype, warp5.dtype)
        ...
        warp4 = self.warp(c24, up_flow5 * 1.25).to(c14.dtype)
        ...
        warp3 = self.warp(c23, up_flow4 * 2.5).to(c13.dtype)
        ...
        warp2 = self.warp(c22, up_flow3 * 5.0).to(c12.dtype)
