...
def create_optimizer(
    ...
    if isinstance(model, torch.nn.Module):
        ...
        # para_model, weight_decay = get_parameters(model['model'])
        # print(para_model)
        parameters = [
            {
                "params": [
            ...
            # {
            #     "params": para_model,
            #     "lr_scale": [p['lr_scale'] for p in para_model if p.requires_grad],
            # }
            # para_model,
            {
                "params": [
                    p
                    for n, p in model["balancer"].named_parameters()
                    if p.requires_grad
                ],
