...
def interpolate_pos_embed_multimae(model, checkpoint_model):
    ...
    for key in matched_keys:
        ...
        if getattr(model.output_adapters, domain, None) is not None:
            ...
            if (orig_H != new_H) or (orig_W != new_W):
                print(
                    f"Key {key.replace('input', 'output')}: Position interpolate from {orig_H}x{orig_W} to {new_H}x{new_W}"
                )
                # print('output_adapter', domain)
                pos_embed_checkpoint = torch.nn.functional.interpolate(
                    pos_embed_checkpoint,
                    size=(new_H, new_W),
                    mode='bicubic',
    ...
    if 'fuse.pos_emb' in checkpoint_model.keys():
        ...
        if (orig_H != new_H) or (orig_W != new_W):
            print(
                f"Key fuse.pos_emb: Position interpolate from {orig_H}x{orig_W} to {new_H}x{new_W}"
            )
            pos_embed_checkpoint_fus = torch.nn.functional.interpolate(
                pos_embed_checkpoint_fus,
                size=(new_H, new_W),
                mode='bicubic',
                align_corners=False,
            )
